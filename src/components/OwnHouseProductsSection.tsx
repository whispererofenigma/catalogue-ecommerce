// components/OwnHouseProductsSection.tsx
import { createClient } from "@/utils/supabase/server";
import ProductCard from "./ProductCard";
import Link from 'next/link';
import { Suspense } from "react";

export default async function OwnHouseProductsSection() {
  const supabase = await createClient();

  // Fetch the 4 most recently updated products
  const { data: products } = await supabase
    .from('products')
    .select('name, slug, price, image_key, categories!inner ( slug )')
    .eq('categories.slug', 'the-looks')
    ...
    <Suspense fallback={<div className="border rounded-2xl p-4 shadow-sm animate-pulse">
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-extrabold text-gray-900">The Looks</h2>
          <p className="mt-2 text-gray-600">Check out our latest and greatest own house creations.</p>
        </div>
        ...
        <div className="text-center mt-12">
          <Link href="/categories/the-looks" className="text-blue-600 hover:text-blue-800 font-semibold">
            View All Products in this Category &rarr;
          </Link>
        </div>
      </section>
    </Suspense>
